...
if [ "$(pwd)" = "/mnt/c/Users/ngehrmann" ]; then
        cd
fi
...
__helm_handle_standard_completion_case() {
    ...
    if ((${#COMPREPLY[*]} == 1)); then
        ...
        __helm_debug "Removed description from single completion, which is now: ${COMPREPLY[0]}"
    else
